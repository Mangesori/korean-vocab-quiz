-- Fix submit_quiz_answers:
-- 1. Restore missing 'word' field that was dropped in 20260508000007
-- 2. Add optional _problem_order parameter so stored answers match the student's shuffled order
...
BEGIN
  ...
  -- First pass: score all problems and build a keyed map
  ...
  LOOP
    ...
    _temp_answers := _temp_answers || jsonb_build_object(_problem_id, jsonb_build_object(
      ...
      'isCorrect', (_user_answer = LOWER(TRIM(_correct_answer))),
  ...
  -- Second pass: build answers array in student order (if provided) or original order
