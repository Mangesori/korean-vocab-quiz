-- submit_quiz_answers 함수 수정
-- 기존 함수는 sentence, translation 등을 포함하지 않았고, 키 이름이 클라이언트와 불일치함 (answer vs userAnswer)
-- 또한 quiz_answers 테이블 대신 quizzes.problems JSON을 직접 순회하여 순서를 보장하고 데이터를 한번에 가져옴
  ...
  LOOP
    ...
    IF _user_answer = LOWER(TRIM(_correct_answer)) THEN
      ...
      _answers_array := _answers_array || jsonb_build_object(
        ...
        'userAnswer', _student_answers->>_problem_id, -- 원본 대소문자 유지
