...
async function callGemini(prompt: string, systemInstruction: string): Promise<string> {
  ...
  const timeoutId = setTimeout(() => controller.abort(), 55000); // 55초 타임아웃 (Edge Function 기본 60초 제한보다 약간 짧게)
...
serve(async (req) => {
  ...
  try {
    ...
    if (reqBody.problems && Array.isArray(reqBody.problems)) {
      ...
      const jsonStr = await callGemini(prompt, systemInstruction);

      const results: GradeResponse[] = JSON.parse(jsonStr);
