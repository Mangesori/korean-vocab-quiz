...
2. **문법 다양성 및 자연스러움 준수 사항 (매우 중요!)**:
   - **제1원칙: 자연스러움 최우선**: 문법 다양성보다 '자연스러운 한국어 문장'이 훨씬 중요합니다. 만약 특정 문법을 사용하여 문장이 어색해진다면, 과감히 기본 문법(서술/종결)을 사용하세요.
   - **자연스러운 범위 내에서의 다양성**: 문맥상 자연스러운 상황이라면, 가능한 한 다양한 카테고리('희망', '추측', '제안' 등)의 문법을 시도하세요.
   - **반복 회피**: 가급적 동일한 문법 패턴이 연속으로 나오지 않도록 신경 써주세요.
