...
const DIFFICULTY_GUIDES: Record<string, string> = {
   ...
   **규칙: 문장을 만들 때 아래 나열된 B1 문법 표현 중 하나를 반드시 포함하세요. 중급 수준의 연결어미와 표현을 사용해주세요.**
   ...
   - 목적/이유: -기 위해서, -(으)려면, -기 때문에, -는 덕분에, -(으)ㄹ까 봐(걱정/이유)
   - 정도/비교: -아/어서 그런지
   - 양보: -아도/어도, -지만, -(으)ㄴ데(도)
   - 나열: -(이)나, -거나
   - 피동/사동: -게 하다
   - 기타: -는 걸 보니까, -는 편이다, -는 중이다
   - 관형사형: -(으)ㄴ, -는, -(으)ㄹ (필수!)
   - 길이: 10-15단어`,
   ...
   **규칙: 문장을 만들 때 아래 나열된 B2 문법 표현 중 하나를 반드시 포함하세요. 고급 연결어미와 피동/사동 표현을 적극적으로 사용해주세요.**
   ...
   - 고급 연결: -(으)므로, -는 통에, -(으)ㄹ수록, -는 한편, -는 반면에, -는 대로, -도록, -는 김에, -는 바람에, -다가는
   - 피동/사동: -게 하다, -게 되다
   - 추측/양태: -(으)ㄹ 텐데, -(으)ㄹ 모양이다, -(으)ㄹ 셈이다, -(으)ㄹ 리가 없다
   - 목적: -(으)ㄹ 겸 (해서)
   - 정도: -(으)ㄹ 만하다, -(으)ㄹ 뿐만 아니라, -(으)ㄹ 뿐이다, -만 못하다, -(으)ㄹ 정도로
   - 이유: -는 탓에, -(으)ㄴ/는 덕분에, -길래
   - 가정/조건: -(으)ㄹ 테니까, -(으)면 되다
   - 후회: -(으)ㄹ걸 (그랬다), -(으)ㄹ 뻔하다
   - 의도: -(으)려다가, -(으)려던 참이다
   - 기타: -는 사이에, -는 수가 있다, -는 수밖에 없다, -(으)ㄴ/는 척하다, -곤 하다, -(으)ㄴ 채로, -다 보니까, -다 보면
   - 관형사형: -(으)ㄴ, -는, -(으)ㄹ
   - 길이: 14-20단어`,
   ...
   **규칙: 문장을 만들 때 아래 나열된 C1 문법 표현 중 하나를 반드시 포함하세요. 격식체와 문어체 표현을 사용하여 고급스러운 문장을 만들어주세요.**
   ...
   - 고급 문법: -던, -(으)ㄴ 바 있다, -(으)로 인해, -에 따라(서), -에 의해(서)
   - 격식체: -ㅂ니다/습니다, -는바, -고자
   - 인용: -다고 하다, -(으)라고 하다, -냐고 하다, -자고 하다
   - 회상: -더니, -더라, -더라고(요), -더라도, -던, -던가요, -던데(요)
   - 강조: -(이)야말로, -는/은커녕, 마저, 조차
   - 나열/선택: -다거나, -(으)ㄹ까 말까 (하다), -든지
   - 비유: -듯이, -다시피
