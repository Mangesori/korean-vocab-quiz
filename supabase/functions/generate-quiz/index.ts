...
const DIFFICULTY_GUIDES: Record<string, string> = {
    "A1": `
   **A1 (TOPIK I 초급 1급)**: 
   - 서술/종결: -이에요/예요, -아/어요, -았/었어요(과거), -지요?(확인)
   ...
   - 수식: 형용사+-(으)ㄴ
   - 단위: 개/병/잔/그릇
   - 길이: 5-8어절(띄어쓰기로 나눈 덩어리 수)`,

    "A2": `
   **A2 (TOPIK I 초중급 2급) - 핵심 문법 필수 사용**:
   **규칙: 문장을 만들 때 아래 나열된 A2 문법 표현 중 하나를 반드시 포함하세요. **
   
   - 서술/종결: -네요(감탄), -다/-ㄴ다/-는다(문어체), N(이)다(문어체)
   - 의문(부드러움): -(으)ㄴ가요?, -나요?, N인가요?
   - 부정: -지 않다(장형 부정)
   - 이유(심화): -(으)니까, -기 때문에, N 때문에, N(이)라서
   - 순서(연속 동작): -아서/어서(순서), -고 나서
   - 동시동작/시간: -(으)면서, -는 동안에, N 동안, -다가(전환), -(으)ㄴ 지(경과), -기 전에, -(으)ㄴ 후에, -았/었을 때
   - 배경/대조: -는데/-(으)ㄴ데(연결), -(으)ㄴ데요, -는데요, N인데요, 형용사+-(으)ㄴ데
   - 추측: -(으)ㄹ까요?(추측), -(으)ㄹ 거예요(추측), -(으)ㄹ 것 같다, -(으)ㄴ/는 것 같다, N인 것 같다
   - 양태/외견: -아/어 보이다, -(으)ㄹ지 모르겠다
   - 제안/의지/권유: -(으)ㅂ시다, -(으)ㄹ래요, -(으)ㄹ게요, -겠어요, -는 게 좋겠다, -는 게 어때요?
   - 계획/의도: -(으)려고
   ...
   - 상태/성향: -는 중이다, N 중이다, -(으)ㄴ/는 편이다
   - 보조용언: -아/어 있다(상태 지속), -아/어 놓다, -아/어 버리다, -아/어다 주다
   - 나열/선택: -거나, (이)나(명사 나열)
   - 조사: 밖에, 마다, 처럼&같이, 중에(서), N 대신
   - 비교: 보다 더
   - 부사형: 형용사+-게
   - 인용(명사): N(이)라고 하다
   - 명사화: -는 것
   - 관형사형: (동사)+-는, (동사)+-(으)ㄴ, (동사)+-(으)ㄹ
   - 지식/인지: -(으)ㄴ/는지 알다, N인지 알다/모르다
   - 길이: 7-10어절(띄어쓰기로 나눈 덩어리 수)`,
