...
// DIFFICULTY_GUIDES는 supabase/functions/_shared/grammar.ts의 GRAMMAR_ITEMS로부터
// renderGuide()가 만든다 — 문장 은행 태그와 이 프롬프트가 같은 기준을 쓰게 하기 위해서다.
// (렌더 결과가 예전 리터럴과 한 글자도 다르지 않음을 scratchpad/extract-grammar.mjs로 검증했다.)
function difficultyGuide(level: string): string {
 ...
 * A1 어휘 목록 주입 블록.
 *
 * 왜 필요한가: 지시문만으로는 어휘가 통제되지 않는다는 게 실사용에서 확인됐다
 * (A1 문제에 `실력`·`늘리다`·`꾸준히` — 전부 3급/B1 — 이 나왔다). §8의
 * "A1 어휘 수준을 준수하되" 한 줄로는 부족하다. 목록을 실제로 보여줘야 한다.
 *
 * **A1에만 넣는다.** 다른 등급은 목록 없이 기존 지시문만 쓴다.
 *
 * 배치는 §3(문법 가이드) 바로 뒤다. 문법 목록 옆에 어휘 목록이 오는 게 읽는 순서로
 * 자연스럽고, 무엇보다 **§8 자기 점검이 프롬프트 맨 끝에 남아야** 하기 때문이다
 * (마지막 지시가 검증이 되게 한다). 3천 자짜리 목록을 뒤쪽에 넣으면 §4·§5 규칙과
 * §8이 밀린다.
 *
...
function a1VocabSection(difficulty: string, subjectNoun: string): string {
  ...
  return `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
§3-2. A1 어휘 목록 — 이 안에서만 쓰세요
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
▶ A1 어휘 목록 — ${subjectNoun} 외에는 이 목록 안에서만 쓰세요
${A1_VOCAB.join(", ")}

· 위 목록에 없는 내용어(명사·동사·형용사·부사)를 쓰지 마세요.
· 조사·어미·의존명사·고유명사(이름·지명)는 목록에 없어도 됩니다.
· 목록 안에서 자연스러운 문장이 안 나오면 단어를 바꾸지 말고 문장 구조를 바꾸세요.
· 단, ${subjectNoun}는 이 제한의 예외입니다 — 목록에 없어도 그대로 쓰세요.
  선생님이 A1을 고르고도 더 높은 등급의 단어를 넣었다면 그건 의도한 것입니다.
`;
}
...
[Step 3 - 문법 카테고리 분산] Step 2에서 선택한 문장의 문법 패턴을 확인하여, 문제 전체에 걸쳐 다양한 카테고리(이유, 시간, 추측, 양보, 연결 등)가 골고루 사용되도록 조정하세요.
  · 단순 종결 회피: "-아/어요", "-습니다" 같은 기본 종결 어미만으로 끝내지 마세요.
...
▶ 동사/형용사 어휘:
  ...
  · sentence의 ( ) 뒤에 문법 요소가 남아있으면 안 됩니다. (문장 부호는 가능)
    특히 보조용언("하다", "싶다", "있다", "되다", "보다")을 ( ) 밖에 남기지 마세요.
    ✗ "서로 의견을 ( ) 해요."   + answer "존중해야"       ← "해요"가 밖에 남았다
    ✓ "서로 의견을 ( )."        + answer "존중해야 해요"
    ✗ "가족하고 ( ) 싶어요."    + answer "시간을 보내고"   ← "싶어요"가 밖에 남았다
    ✓ "가족하고 ( )."           + answer "시간을 보내고 싶어요"
...
· 복합 구성: "기본 문법 + 종결 어미" 형식.
  **answer가 종결형(-아/어요, -습니다, -았/었어요 …)으로 끝나면 종결 어미를 " + "로 반드시 붙이세요.**
  기본 문법만 적고 종결 어미를 빠뜨리는 것이 가장 흔한 실수입니다.
  예: "가기로 했습니다"     → "-기로 하다 + 습니다"
  예: "할 수 있었어요"      → "-(으)ㄹ 수 있다 + 았/었어요"
  예: "존중해야 해요"       → "-아야/어야 하다 + 아/어요"   (✗ "-아야/어야 하다" 만 쓰면 안 됨)
  예: "남아 있어요"         → "-아/어 있다 + 아/어요"      (✗ "-아/어 있다" 만 쓰면 안 됨)
  예: "시간을 보내고 싶어요" → "-고 싶다 + 아/어요"         (✗ "-고 싶다" 만 쓰면 안 됨)
...
const generateSimplePrompt = (words: string[], difficulty: string, languageName: string, includeShort = false) => {
  ...
  const fullGuide = shuffleGrammarGuide(difficultyGuide(difficulty));
  // 재생성도 학생에게 그대로 나가는 문장이다 — 여기서 통제를 빼면 "마음에 안 들어
...
[문법 가이드 - ${difficulty}]
${fullGuide}
→ 위 목록에서 하나를 골라 활용하세요. 단순 종결(-아/어요)만으로 끝내지 마세요.
${a1Vocab}
[빈칸·정답 규칙]
