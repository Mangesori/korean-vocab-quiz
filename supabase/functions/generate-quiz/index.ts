...
2. **문법 다양성 필수 준수 사항 (매우 중요!)**:
   - **중복 최소화**: 동일한 문법 형태(예: -아요/어요, -아서/어서)를 전체 문제의 20% 이상 사용하지 마세요. 가능한 한 모든 문제에 서로 다른 문법을 적용하려고 시도하세요.
   - **카테고리 순환 활용**: 위에서 정의한 '희망', '추측', '경험', '허락', '변화' 등 다양한 카테고리의 문법을 골고루 섞어서 사용하세요. 단순히 '서술/종결'만 계속 사용하면 안 됩니다.
   - **우선순위**: 문맥이 허용하는 한, 가장 기본적인 문법(예: -아요/어요)보다는 특정한 의미를 가진 문법(예: -기로 하다, -(으)ㄹ 수 있다)을 우선적으로 선택하여 풍부한 표현을 만드세요.
